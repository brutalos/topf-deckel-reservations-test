...
export default function AdminReservationsPage({ adminKey }: { adminKey: string }) {
    ...
    const [stats, setStats] = useState({ totalGuests: 0, seated: 0, pending: 0, freeTables: 10 });
    ...
    const fetchStats = async () => {
        try {
            ...
            if (res.ok) {
                ...
                json.assignments.forEach((a: any) => uniqueReservations.set(a.reservation.id, a.reservation));
                
                const resList = Array.from(uniqueReservations.values());
                const seated = resList.filter(r => r.status === 'SEATED').length;
                const totalGuests = resList.reduce((acc, r) => acc + r.partySize, 0);
                const pending = resList.filter(r => r.status === 'CONFIRMED').length;
                
                setStats({ 
                    totalGuests, 
                    seated, 
                    pending, 
                    freeTables: 10 - new Set(json.assignments.map((a: any) => a.tableId)).size 
                });
            }
        } catch (error) {}
    };
    ...
    return (
        <div className="min-h-screen bg-zinc-50 pt-24 pb-12 px-6">
            <div className="max-w-7xl mx-auto space-y-8">
                ...
                {/* Main View */}
                {view === 'timeline' ? (
                    <TimelineView storeId={storeId} date={date} adminKey={adminKey} />
                ) : (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                    <Card className="bg-white border-zinc-200 shadow-sm">
                        ...
                        <CardContent className="p-4 pt-2">
                            <div className="text-3xl font-black text-zinc-900">{stats.totalGuests}</div>
                    ...
                    <Card className="bg-white border-zinc-200 shadow-sm">
                        ...
                        <CardContent className="p-4 pt-2">
                            <div className="text-3xl font-black text-zinc-900">{stats.seated}</div>
                            <p className="text-[10px] text-zinc-500 mt-1">Ausstehend: {stats.pending}</p>
                        </CardContent>
                    </Card>
                    <Card className="bg-white border-zinc-200 shadow-sm">
                        <CardHeader className="p-4 pb-0">
                            <CardTitle className="text-xs font-bold text-zinc-500 uppercase tracking-wider flex items-center gap-2">
                                <MapPin className="w-4 h-4" /> Belegte Tische
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="p-4 pt-2">
                            <div className="text-3xl font-black text-zinc-900">{10 - stats.freeTables}</div>
